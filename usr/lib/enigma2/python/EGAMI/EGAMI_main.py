...
from Screens.Console import Console as EGConsole
...
class EgamiMainPanel(Screen):
    ...
    def keyOk(self):
        ...
        if selected == _('EGAMI Cam Center'):
            ...
            self.session.open(EGConsole, _('EGAMI - Show Filesystem Mounts'), ['mount'])
            ...
            self.session.open(EGConsole, _('EGAMI - Show all connected sockets from ip-stack'), ['who'])
        elif selected == _('Show Routing Table'):
            self.session.open(EGConsole, title=_('EGAMI show network configuration and routing table'), cmdlist=['ip addr; iwconfig; echo ------------------------------------------------; route -n; echo ================================================; cat /etc/resolv.conf'])
            ...
            self.session.open(EGConsole, _('EGAMI - show network stats'), ['netstat -t -u'])
...
class EGProcessInfo(Screen):
    ...
    def showDetails(self):
        ...
        if val:
            ...
            self.session.open(EGConsole, _('Details'), [cmd])
...
class EGScript(Screen):
    ...
    def mygo(self):
        ...
        self.session.open(EGConsole, title=mytitle, cmdlist=[mysel2])
...
class EGStreamInfo(Screen):
    ...
    def go(self):
        ...
        if returnValue is not None:
            if returnValue is '1':
                self.session.open(EGConsole, _('Stream advanced info - PAT'), ['/usr/bin/dvbsnoop -n 1 -hideproginfo 0x0000'])
            elif returnValue is '2':
                self.session.open(EGConsole, _('Stream advanced info - CAT'), ['/usr/bin/dvbsnoop -n 1 -hideproginfo 0x0001'])
            elif returnValue is '3':
                self.session.open(EGConsole, _('Stream advanced info - NIT'), ['/usr/bin/dvbsnoop -n 1 -hideproginfo 0x0010'])
            elif returnValue is '4':
                self.session.open(EGConsole, _('Stream advanced info - SDT/BAT'), ['/usr/bin/dvbsnoop -n 1 -hideproginfo 0x0011'])
            elif returnValue is '5':
                self.session.open(EGConsole, _('Stream advanced info - EIT/TOT/TDT'), ['/usr/bin/dvbsnoop -n 1 -hideproginfo 0x0012'])
            elif returnValue is '6':
                self.session.open(EGConsole, _('Stream advanced info - PMT'), ['/usr/bin/dvbsnoop -if /tmp/pmt.tmp -hideproginfo'])
            elif returnValue is '7':
                self.session.open(EGConsole, _('Stream advanced info - ECM'), ['cat /tmp/ecm.info'])
            elif returnValue is '8':
                self.session.open(EGConsole, _('Stream advanced info - Help'), ['/usr/bin/dvbsnoop -help'])
        return
...
class EGSwapManager(ConfigListScreen, Screen):
    ...
    def __init__(self, session):
        ...
        self.size = ConfigSelection(default=self.def_size, choices=[('8192', _('8 MB')),
         ('16384', _('16 MB')),
         ('32768', _('32 MB')),
         ('65536', _('64 MB')),
         ('131072', _('128 MB')),
         ...
         ('524288', _('512 MB')),
         ('1048576', _('1 GB'))])
        self.place = ConfigSelection(default=self.def_place, choices=['/media/usb',
         '/media/hdd',
         '/media/cf',
         '/media/mbX',
         '/media/ba'])
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions', 'CiSelectionActions'], {'ok': self.cancel,
         'cancel': self.cancel,
         'red': self.active_swap,
         'green': self.deactive_swap,
         'yellow': self.remove_swap,
         'blue': self.create_swap}, -2)
    ...
    def createInfo(self):
        system('cat /proc/swaps > /tmp/swaps.tmp')
